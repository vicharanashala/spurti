...
import FactionWar from './FactionWar.jsx';
...
function StudentView({ profile, onBack }) {
  ...
  const [showBeatBest, setShowGhost] = useState(false);
    ...
    <main className="page compact">
      <header className="topbar">
        ...
        <div>
          ...
          <h1>{student.name}</h1>
          <SkillBadges email={student.email} mode="header" />
        </div>
      ...
      <button
        className="fw-entry-btn"
        onClick={() => setShowFaction(true)}
      >
        ⚔️ Faction Wars
      </button>
      <StudentPulse profile={profile} badges={badges} nextActions={nextActions} />
      <Tabs tab={tab} setTab={setTab} tabs={[['bank','SP Bank'], ['polls','Polls'], ['leaderboard','Leaderboard'], ['skills','🌳 Skill Tree']]} />
...
{tab === 'skills' && <SkillTree />}
      ...
      {showFaction && (
        <div className="fw-overlay">
          <FactionWar onClose={() => setShowFaction(false)} />
        </div>
      )}
    </main>
  );
