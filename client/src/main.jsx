...
// Keyed by the stable `_id` returned by the server (leaderboard rows and search
// matches both carry it), never by maskedEmail, so two peers whose masked emails
// collide can't overwrite each other's star/bookmark state.
...
function PeersTab({ profile }) {
  ...
  // Leaderboard rows (exclude self), enriched with the stable `_id` the server
  // now includes on every row.
