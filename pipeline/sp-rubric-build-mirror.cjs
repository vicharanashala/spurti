  ...
  for (const [cand, info] of candidates) {
    ...
    if (qPens && qPens.length) {
      ...
      for (const d of [...penByDay.keys()].sort()) {
        ...
        // Penalty rows pushed earlier in this loop are already inside `rows`, so
        // their negative deltas are already subtracted by the reduce — do NOT
        // subtract them again, or the clamp undercounts a student's balance and
        // rejects penalties they should be charged.
