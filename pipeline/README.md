...
> **CURRENT SCORER (2026-06-28 →).** SP is scored by the **sakshi-side** mirror
> rubric **`sp-rubric-build-mirror.cjs`**, run from **`~/spurti/sp-refresh.sh`**
> (repo root) on the 6-hourly cron. It reads only the `sakshi_spurti` mirrors
> (`zoom_meetings`, `zoom_attendance`, `zoom_polls`, `spandan_polls`,
> `candidates`, `students`, `act_spa_*`, `act_query_reviews`) — no Zoom
> credentials, no live Zoom Reports API. Rules: see CONTEXT.md "SP Calculation".
>
> **RETIRED samagama-side scorer.** The former live-API `sp-rubric-build.js`
> (which fetched Zoom reports directly and was the cause of the 2026-06-27
> regression — sessions older than Zoom's ~3–4 week retention came back empty
> and were scored 0) is **retired**. Do NOT reintroduce it or its
> `sp-pipeline.sh`/`sp-pipeline.cron` APPLY stage. Samagama's only remaining job
> is feeding the mirrors — the Zoom-data mirror (`zoom-update.js` +
> `sync-sakshi-zoom-mirror.js`) and the expanded `candidates` roster
> (`sync-collaborator-mirrors.js`). See `HANDOFF_MIRROR_AND_ROSTER.md`.
>
> The `+5/-5` logic that used to live in `server/scripts/` is likewise retired.

> **Deployment note.** The samagama-side live copy runs from
> `/var/samagama/server/` as the `samagama` OS user. The files here are a
...
Scoring runs on the **sakshi side**; the samagama side only feeds mirrors.

```
Zoom  ──zoom-update.js──►  zoom_data.{meetings,attendance,polls,summaries,transcripts}
                                  │      └──────────────►  chatengine (candidates roster via
                                  ▼                          sync-collaborator-mirrors.js)
                 sync-sakshi-zoom-mirror.js (chained in zoom-update.js)
                                  │
                                  ▼
                        sakshi_spurti.zoom_*  (ZOOM-DATA MIRROR — samagama's feed)
                                  │
        ┌─────────────────────────┴─────────────────────────┐
        ▼ (SAKSHI side: cron 6-hourly)                      ▼
   sp-rubric-build-mirror.cjs  (THE SCORER, reads mirrors)  chatengine.spledgers
   reads zoom_* / spandan_polls / act_* / candidates        + User.spPoints (via
        │  wipe-and-rebuild → writes ledger                  sync-spurti-from-sakshi.js)
        ▼
   sakshi_spurti.students.totalSp + sptransactions
        │
        ▼ (sp-refresh.sh, non-fatal derived steps)
   sync-attendance-records / sync-poll-records (repo-root sp-refresh.sh)
        ▼
   sakshi_spurti.{attendancerecords,pollrecords}
        │
        ▼
   Spurti web app (../server) reads sakshi_spurti  ──►  https://samagama.in/spurti/
```

## Scoring rubric (implemented in `sp-rubric-build-mirror.cjs`)

The full current band/tier rubric — `initial`, `attendance`, `poll`, `spa`,
`query` categories (plus preserved `manual`/`peer_faq`), the attendance-window
evening cutover, the Spandan hybrid attendance from Day 64, percentiled Spandan
poll correctness, SPA validated endorsements, and the query-answer pay + forward
penalty — is documented in **CONTEXT.md → "SP Calculation — band/tier rubric
(current, 2026-08)"**. This README keeps only the historical outline that the
retired `sp-rubric-build.js` implemented:
