...
import { getHighestTitlesPerBranch } from '../services/skillTree.js';
...
router.get('/', async (req, res) => {
 ...
 // Batch-fetch unlocks for everyone on the board and attach the highest
 // title per branch. Pure decoration.
 const wlbUnlocks = await SkillTreeUnlock
 .find({ email: { $in: rows.map(r => r.email) } })
 .select('email branch nodeIndex')
 .lean();
 const wlbTitlesByEmail = {};
 for (const e of rows.map(r => r.email)) wlbTitlesByEmail[e] = {
 consistency: [], curiosity: [], momentum: [], excellence: [],
 };
 for (const u of wlbUnlocks) {
 if (wlbTitlesByEmail[u.email] && wlbTitlesByEmail[u.email][u.branch]) {
 wlbTitlesByEmail[u.email][u.branch].push(u.nodeIndex);
 }
 }
 for (const e of rows.map(r => r.email)) {
 wlbTitlesByEmail[e] = getHighestTitlesPerBranch(wlbTitlesByEmail[e]);
 }
 ...
 skillTitles: wlbTitlesByEmail[row.email] || null,
 });
