...
import { batchLegacyTitlesByEmail } from '../services/skillBadgeAdapter.js';
...
/* ── Motivational message (no ghost wording) ───────── */
function ghostMessage(status, spDiff, today) {
  const late = ['Thu','Fri','Sat','Sun'].includes(today);
  if (status === 'no-ghost')
    return "No benchmark yet \u2014 the race begins after your first full week! \uD83E\uDD47";
  if (status === 'tied')
    return "Perfectly tied with last week. One session tips the balance. \u26A1";
  if (status === 'ahead') {
    if (spDiff >= 20)
      return `You're crushing last week by ${spDiff} SP. Unstoppable. \uD83D\uDD25`;
    if (spDiff >= 10)
      return `${spDiff} SP ahead of last week. Keep the pressure on. \uD83D\uDCAA`;
    return `${spDiff} SP ahead of last week. Don't let it catch up. \uD83C\uDFC3`;
  }
  // behind
  if (late && Math.abs(spDiff) > 15)
    return `Last week is ${Math.abs(spDiff)} SP ahead. Still time to close the gap. \u23F0`;
  if (Math.abs(spDiff) <= 5)
    return `Only ${Math.abs(spDiff)} SP behind last week. You can catch it today. \u26A1`;
  return `Last week earned ${Math.abs(spDiff)} more SP at this point. Chase it. \uD83E\uDD47`;
}
...
router.get('/', async (req, res) => {
  try {
    ...
    // Highest earned title per branch — used by the Ghost Race header.
    // Adapter reads from the new flat-list StudentSkillTree and returns
    // the legacy {consistency, curiosity, momentum, excellence} shape,
    // so the existing GhostRace.jsx header renders unchanged.
    const titlesByEmail = await batchLegacyTitlesByEmail([email]);
    const skillTitles = titlesByEmail[email] || {
    ...
    return res.json({
      ...
      // Decorative: highest earned title per branch for this student.
      // Lets the Ghost Race header show off the wearer's accomplishments.
      skillTitles,
    });
  } catch (err) {
